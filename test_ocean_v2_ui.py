...
class MainWindow(QMainWindow):
    ...
    def _on_get_config(self):
        ...
        for i, route_data in enumerate(routes_data):
            ...
            self._log(f"  脚本 (买入-到达): {route_data.get('script_buy_arrived', '')}")
            self._log(f"  脚本 (买入-未到达): {route_data.get('script_buy_not_arrived', '')}")
            self._log(f"  脚本 (卖出-到达): {route_data.get('script_sell_arrived', '')}")
            self._log(f"  脚本 (卖出-自动): {route_data.get('script_sell_auto', '')}")
            self._log("-" * 40)
        ...
        for route_data in routes_data:
            ...
            route_config = OceanRouteConfig(
                ...
                buy=buy,
                sell=sell,
                script_buy_arrived=route_data.get('script_buy_arrived', ''),
                script_buy_not_arrived=route_data.get('script_buy_not_arrived', ''),
                script_sell_arrived=route_data.get('script_sell_arrived', ''),
                script_sell_auto=route_data.get('script_sell_auto', ''),
            )
